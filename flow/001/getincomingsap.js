...
router.post('/tblSAPGoodReceive_get', async (req, res) => {
    ...
    let querypost = await mongodbMAIN.find("SAPincoming", "tblSAPPostIncoming", { $and: [{ "Plant": "BP12GAS" }] });
...
router.post('/refreshdatafromSAP-BP12GAS', async (req, res) => {
    console.log("-------- refreshdatafromSAP-BP12GAS --------");
    // console.log(req.body);
    let input = req.body;
    //--------------------------------->
    let output = [];
        ...
        try {
            ...
            if (resp.status == 200) {
                ...
                if (returnDATA[`hasData`].toString().toUpperCase() === `TRUE`) {
                    ...
                    let upd = await mongodbMAIN.update("SAPincoming", "tblSAPGoodReceive", { "Plant": 'BP12GAS' }, { $set: UPDATEdata });
                    output = 'OK01'
                }
        ...
        try {
            ...
            if (resp.status == 200) {
                ...
                if (returnDATA[`hasData`].toString().toUpperCase() === `TRUE`) {
                    ...
                    let upd = await mongodbMAIN.update("SAPincoming", "tblSAPPostIncoming", { "Plant": 'BP12GAS' }, { $set: UPDATEdata });
                    output = 'OK02'
                }
